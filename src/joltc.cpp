...
/* RagdollSettings - Part Management */
...
{
	// Convert JPH_Mat4 arrays to JPH::Mat44 arrays
	int count1 = AsSkeleton(skeleton1)->GetJointCount();
	int count2 = AsSkeleton(skeleton2)->GetJointCount();

	JPH::Array<JPH::Mat44> pose1(count1);
	JPH::Array<JPH::Mat44> pose2(count2);

	for (int i = 0; i < count1; i++)
		pose1[i] = ToJolt(&neutralPose1[i]);
	for (int i = 0; i < count2; i++)
		pose2[i] = ToJolt(&neutralPose2[i]);

	AsSkeletonMapper(mapper)->Initialize(AsSkeleton(skeleton1), pose1.data(), AsSkeleton(skeleton2), pose2.data());
}
...
{
	int count = AsSkeleton(skeleton2)->GetJointCount();
	JPH::Array<JPH::Mat44> pose(count);
	for (int i = 0; i < count; i++)
		pose[i] = ToJolt(&neutralPose2[i]);

	AsSkeletonMapper(mapper)->LockAllTranslations(AsSkeleton(skeleton2), pose.data());
}
...
void JPH_SkeletonMapper_LockTranslations(JPH_SkeletonMapper* mapper, const JPH_Skeleton* skeleton2, const bool* lockedTranslations, const JPH_Mat4* neutralPose2)
{
	...
	AsSkeletonMapper(mapper)->LockTranslations(AsSkeleton(skeleton2), lockedTranslations, pose.data());
}
...
{
	// Get the mappings to determine skeleton sizes
	const auto& mappings = AsSkeletonMapper(mapper)->GetMappings();
	if (mappings.empty())
		return;

	// We need to determine the sizes - this is a limitation of the C API
	// The user must ensure arrays are properly sized
	// For now, we assume the caller knows the correct sizes
	// This is a direct pass-through to the Jolt API
...
/* Ragdoll - Body/Constraint Access */
